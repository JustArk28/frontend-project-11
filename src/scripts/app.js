...
const app = () => {
  ...
  const isFeedExists = (feedTitle, feedDescription) => {
    return state.feeds.some(
      (feed) => feed.title === feedTitle && feed.description === feedDescription
    );
  };
  ...
  const isPostExists = (postTitle, postLink) => {
    return state.posts.some(
      (post) => post.title === postTitle && post.link === postLink
    );
  };
  ...
  const rssData = () => {
    ...
    axios
      ...
      .then((response) => {
        ...
        const newPosts = createPosts(newDom).filter(
          (post) => !isPostExists(post.title, post.link)
        );
